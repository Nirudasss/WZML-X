...
WORKER_URL = "https://antibypass.tellycloudapi.workers.dev/create"

async def short_url(longurl, attempt=0):
    if not longurl:
        return longurl
    if attempt >= 4:
        LOGGER.warning(f"Max attempts reached â†’ {longurl}")
        return longurl
    disable_warnings()
    cget = create_scraper().request
    # ----------------------------------------------------
    # METHOD 1 â†’ CLOUDFLARE WORKER (NEW API)
    # ----------------------------------------------------
    try:
