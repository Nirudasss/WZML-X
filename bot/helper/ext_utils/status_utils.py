...
async def get_specific_tasks(status, user_id):
    tasks_to_check = [tk for tk in task_dict.values() if not user_id or tk.listener.user_id == user_id]
    ...
    for tk in tasks_to_check:
        ...
        if tk in coro_tasks:
            coro_index += 1
        if status == "All" or st == status or (status == MirrorStatus.STATUS_DOWNLOAD and st not in STATUSES.values()):
            result.append(tk)
    return result
    ...
    for task in tasks[start:start + STATUS_LIMIT]:
        try:
            ...
            msg += (
                ...
                f"┣ 👤 <b>User</b> : {escape(getattr(user, 'first_name', 'Unknown'))}\n"
                f"┣ 🆔 <b>ID</b> : {getattr(user, 'id', 'Unknown')}\n"
                f"┣ 🔗 <b>Source</b> : {source}\n"
                f"┗ 🛑 <b>Stop</b> : <i>/{BotCommands.CancelTaskCommand[1]}_{task.gid()}</i>\n\n"
            )

        except Exception as e:
            msg += f"⚠️ <b>Error fetching task info:</b> {str(e)}\n\n"
